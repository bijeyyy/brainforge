...
export default function MockExam() {
  ...
  const [examToDelete, setExamToDelete] = useState<Quiz | null>(null);
  ...
  async function createExam() {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        passing_percent: Math.min(100, Math.max(1, newPassing)),
      })
      .select()
      .single();
    if (error) {
      alert("Could not create exam: " + error.message);
      return;
    }
    setNewTitle("");
    setShowNew(false);
  ...
  function requestDeleteExam(exam: Quiz) {
  ...
  function submit() {
    ...
    if (
      unanswered > 0 &&
      !confirm(`${unanswered} question(s) unanswered. Submit anyway?`)
    )
      return;
    submitRef.current();
  }
  ...
  if (active && finished) {
    ...
    questions.forEach((q, i) => {
      ...
      e.total++;
      if (answers[i] === q.correct_index) e.right++;
      topics.set(t, e);
    });
    return (
      <div className="page-in max-w-xl mx-auto">
        <Card className="p-8 text-center mb-4">
          ...
          <Pill tone={passed ? "green" : "red"}>
            {passed ? "PASSED" : "FAILED"} · needs {active.passing_percent}%
          </Pill>
          <p className="text-sm text-slate-500 mt-2">
            {score} of {questions.length} correct · +{score * 8} XP
          </p>
        </Card>
    ...
    return (
      <div className="page-in">
        <div className="flex items-center gap-4 mb-5 flex-wrap">
          ...
          <span
            className={`ml-auto font-display font-semibold tabular-nums px-3 py-1 rounded-full border text-sm
            ${low ? "text-red-600 border-red-200 animate-pulse" : mid ? "text-amber-600 border-amber-200" : "border-slate-200"}`}
          >
            ⏱ {fmt(seconds)}
          </span>
          <Btn variant="danger" onClick={submit}>
            Submit exam
          </Btn>
        </div>
        ...
        <div className="grid grid-cols-[1fr_240px] max-md:grid-cols-1 gap-4">
          <Card className="p-7">
            <div className="flex justify-between items-start">
              ...
              >
                <Flag
                  size={18}
                  fill={flags.has(index) ? "currentColor" : "none"}
                />
              </button>
            </div>
            <h2 className="text-lg font-semibold my-4 leading-relaxed">
              {q.prompt}
            </h2>
            {q.options.map((opt, i) => (
              <button
              ...
              >
                <span
                  className={`w-6 h-6 rounded-md border grid place-items-center text-xs font-semibold shrink-0
                  ${answers[index] === i ? "bg-primary border-primary text-white" : "border-slate-300"}`}
                >
          ...
          <Card className="p-4 h-fit">
            ...
            <div className="text-xs text-slate-500 mt-3 space-y-1">
              <div>
                {Object.keys(answers).length} answered · {flags.size} flagged
              </div>
              <div>
  ...
  return (
    <div className="page-in">
      <PageHeader
        title="Mock Exams"
        sub="Real exam simulation — timed, no feedback until you submit"
        action={
        ...
        <Card className="p-4 mb-5 flex gap-3 items-center flex-wrap">
          ...
          <label className="flex items-center gap-1.5 text-sm text-slate-500">
            <input
              type="number"
              min={1}
              className="h-10 w-20 px-3 rounded-[10px] border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-primary"
              value={newDuration}
      ...
      {exams.length === 0 ? (
        <Empty
          ...
          sub="Click New exam above to build your own."
        />
      ) : (
        <div className="grid grid-cols-3 max-lg:grid-cols-2 max-md:grid-cols-1 gap-4">
          {exams.map((e) => (
            <Card key={e.id} className="p-5">
              <div className="w-11 h-11 rounded-xl bg-red-50 grid place-items-center text-xl mb-3">
                📝
              </div>
              <h3 className="font-semibold">{e.title}</h3>
              <p className="text-xs text-slate-500 mb-3">
                {e.questions?.[0]?.count ?? 0} items · {e.duration_minutes} min
                · pass at {e.passing_percent}%
              </p>
              <div className="flex gap-2">
                <Btn
                  className="flex-1"
                  onClick={() => start(e)}
                  disabled={(e.questions?.[0]?.count ?? 0) === 0}>
                  Start exam →
                </Btn>
                <Btn
                  variant="ghost"
                  onClick={() => setBuilding(e)}
      ...
      <AlertDialog open={!!examToDelete} onOpenChange={(open) => !open && setExamToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete "{examToDelete?.title}"?</AlertDialogTitle>
            <AlertDialogDescription>
              This will also delete all its questions and attempt history. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDeleteExam}>
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
