...
export default function Quizzes() {
  ...
  async function loadQuizzes() {
    const { data } = await supabase.from('quizzes').select('*, questions(count)').eq('is_exam', false).order('created_at')
    setQuizzes((data as Quiz[]) ?? []); setLoading(false)
  }
  ...
  async function deleteQuiz(quiz: Quiz) {
    if (!confirm(`Delete "${quiz.title}"? This will also delete all its questions and attempt history. This cannot be undone.`)) return
    const { error } = await supabase.from('quizzes').delete().eq('id', quiz.id)
    if (error) { alert('Could not delete quiz: ' + error.message); return }
  ...
  async function handlePdfUpload(file: File) {
    ...
    try {
      ...
      const { title, questions: generated } = await res.json() as {
        title: string
        questions: { prompt: string; options: string[]; correct_index: number; explanation: string | null; topic: string | null }[]
      }

      if (!generated?.length) {
        throw new Error('The AI could not find enough content to make a quiz from this PDF.')
      }
      ...
      setPdfError(err.message ?? 'Something went wrong generating a quiz from that PDF.')
    } finally {
  ...
  async function start(quiz: Quiz) {
    ...
    setActive(quiz); setIndex(0); setAnswers({}); setPicked(null); setFinished(false)
  ...
  async function next() {
    ...
    if (index + 1 < questions.length) {
      ...
      const score = questions.reduce((s, q, i) => s + (answers[i] === q.correct_index || (i === index && picked === q.correct_index) ? 1 : 0), 0)
      const duration = Math.round((Date.now() - startedAt) / 1000)
      await supabase.from('quiz_attempts').insert({
        user_id: (await supabase.auth.getUser()).data.user!.id,
        quiz_id: active!.id, mode: 'quiz', score, total: questions.length, duration_seconds: duration,
      })
  ...
  // ── Question builder ──
  if (building) {
    return <QuizBuilder quizId={building.id} title={building.title} onBack={() => { setBuilding(null); loadQuizzes() }} />
  }
    ...
    return (
      <div className="page-in max-w-md mx-auto text-center pt-14">
        <Card className="p-8">
          ...
          <p className="text-sm text-slate-500 mb-4">{score} of {questions.length} correct · +{score * 5} XP</p>
          <div className="flex gap-2 justify-center">
            <Btn variant="ghost" onClick={() => setActive(null)}>Back to quizzes</Btn>
  ...
  if (active) {
    ...
    if (!q) return <Empty icon="❓" title="This quiz has no questions" sub="Add questions manually or generate them from a PDF." action={<Btn onClick={() => setActive(null)}>Back</Btn>} />
    return (
      <div className="page-in max-w-2xl mx-auto">
        <div className="flex items-center gap-3 text-sm text-slate-500 mb-4">
          <button onClick={() => setActive(null)} className="hover:text-slate-900">← Exit</button>
          <span>Question {index + 1} of {questions.length}</span>
          <ProgressBar value={((index + 1) / questions.length) * 100} className="flex-1" />
        </div>
        <Card className="p-7">
          <div className="text-[11px] font-semibold uppercase tracking-wider text-primary mb-1.5">{q.topic ?? 'Question'}</div>
          <h2 className="text-lg font-semibold mb-5 leading-relaxed">{q.prompt}</h2>
          {q.options.map((opt, i) => {
            ...
            return (
              <button key={i} onClick={() => pick(i)} disabled={picked !== null}
                ...
                <span className="w-6 h-6 rounded-md border border-slate-300 grid place-items-center text-xs font-semibold shrink-0">{String.fromCharCode(65 + i)}</span>
                {opt}
              </button>
            )
          })}
        ...
        <div className="flex justify-end mt-4">
          <Btn onClick={next} disabled={picked === null}>{index + 1 === questions.length ? 'Finish ✓' : 'Next →'}</Btn>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="page-in">
      <PageHeader
        title="Quizzes" sub="Practice mode — instant feedback after every answer"
        action={<Btn onClick={() => setShowNew(v => !v)}><Plus size={16} /> New quiz</Btn>}
      />
      {showNew && (
      ...
      <Card className="p-5 mb-5">
        ...
        <p className="text-xs text-slate-500 mb-3">Upload your notes or a reviewer and the AI will turn them into a multiple-choice quiz automatically.</p>
        <label className={`inline-flex items-center justify-center gap-2 h-10 px-3.5 rounded-[10px] border border-dashed text-sm cursor-pointer transition
      ...
      {quizzes.length === 0 ? (
        <Empty icon="❓" title="No quizzes yet" sub="Click New quiz to build your own, or generate one from a PDF above." />
      ) : (
        <div className="grid grid-cols-3 max-lg:grid-cols-2 max-md:grid-cols-1 gap-4">
          {quizzes.map(qz => (
            <Card key={qz.id} className="p-5">
              <div className="flex justify-between items-start mb-3">
                <div className="w-11 h-11 rounded-xl bg-primary-50 grid place-items-center text-xl">❓</div>
                <Pill tone="indigo">Practice</Pill>
              </div>
              <h3 className="font-semibold">{qz.title}</h3>
              <p className="text-xs text-slate-500 mb-3">{qz.questions?.[0]?.count ?? 0} questions · ~{qz.duration_minutes} min</p>
              <div className="flex gap-2">
                <Btn className="flex-1" onClick={() => start(qz)} disabled={(qz.questions?.[0]?.count ?? 0) === 0}>Start →</Btn>
                <Btn variant="ghost" onClick={() => setBuilding(qz)} title="Add or edit questions"><Pencil size={15} /></Btn>
                <Btn variant="ghost" onClick={() => deleteQuiz(qz)} title="Delete quiz" className="text-slate-300 hover:text-red-500">
                  <Trash2 size={15} />
                </Btn>
              </div>
            </Card>
