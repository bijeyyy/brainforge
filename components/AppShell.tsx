...
export default function AppShell({ children }: { children: ReactNode }) {
    ...
    <>
      {nav.map(group => (
        <div key={group.section}>
          <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 px-3 pt-4 pb-1.5">{group.section}</div>
            ...
            return (
              <Link key={item.to} href={item.to}
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-[10px] text-sm font-medium transition
                  ${active ? 'bg-primary-50 text-primary' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}>
                <item.icon size={18} className={item.label === 'AI Assistant' ? 'text-accent' : ''} />
                {item.label}
                {item.label === 'AI Assistant' && (
                  <span className="ml-auto text-[10px] font-semibold bg-accent-100 text-accent px-1.5 py-px rounded-full">AI</span>
                )}
              </Link>
            )
          })}
  ...
  const ThemeToggle = () => (
    <button
      onClick={toggleTheme}
      className="flex items-center gap-3 px-3 py-2 mb-1 rounded-[10px] text-sm font-medium text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition"
    >
  ...
  const StreakCard = () => (
    <div className="flex items-center gap-2.5 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2.5 mb-2">
      <Flame size={18} className="text-amber-500" />
      <div className="text-sm">
        <b>{profile?.streak ?? 0}-day streak</b>
  ...
  const AccountRow = () => (
    <div className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-slate-50">
      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-amber-400 to-red-400 grid place-items-center text-white font-semibold text-sm ring-2 ring-primary ring-offset-2 shrink-0">
        {(profile?.full_name ?? 'U')[0]}
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-[13px] font-semibold truncate">{profile?.username ?? profile?.full_name ?? 'Learner'}</div>
        <div className="text-xs text-slate-500">Lv {levelFromXp(profile?.xp ?? 0)} · {levelProgress(profile?.xp ?? 0)}%</div>
      </div>
      <button title="Log out" onClick={() => supabase.auth.signOut()} className="text-slate-400 hover:text-red-500 shrink-0">
        <LogOut size={16} />
      </button>
    </div>
  )
  ...
  return (
    <ProfileContext.Provider value={{ profile, refreshProfile }}>
      <div className="flex h-screen overflow-hidden">
        ...
        <header className="lg:hidden fixed top-0 inset-x-0 z-30 h-14 bg-white border-b border-slate-200 flex items-center gap-3 px-4">
          <button
            onClick={() => setDrawerOpen(true)}
            aria-label="Open menu"
            className="p-2 -ml-2 rounded-lg text-slate-500 hover:bg-slate-50 hover:text-slate-900"
          >
            <Menu size={22} />
          </button>
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <span className="text-primary-600 font-display font-semibold text-base truncate">BrainForge AI</span>
          </div>
          <div className="flex items-center gap-1.5 text-amber-500 shrink-0">
            <Flame size={18} />
            <span className="text-sm font-semibold text-slate-700">{profile?.streak ?? 0}</span>
          </div>
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-red-400 grid place-items-center text-white font-semibold text-xs ring-2 ring-primary ring-offset-1 shrink-0">
            {(profile?.full_name ?? 'U')[0]}
          </div>
        </header>
        ...
        {drawerOpen && (
          <div className="lg:hidden fixed inset-0 z-40">
            <div
              ...
              aria-hidden="true"
            />
            <aside className="absolute left-0 top-0 h-full w-[280px] max-w-[85vw] bg-white flex flex-col p-4 shadow-xl">
              <div className="flex items-center justify-between pb-5">
                <div className="flex items-center gap-2.5 px-2">
                  <div className="w-9 h-9 rounded-[10px] bg-gradient-to-br from-primary-600 to-accent grid place-items-center text-white font-bold">B</div>
                  <span className="text-primary-600 font-display font-semibold text-lg">BrainForge AI</span>
                </div>
                <button
                  onClick={() => setDrawerOpen(false)}
